...
alldata <- read.delim("demo_independent.txt", header=TRUE, dec=".") #Define the name of the tab delimited txt file (or .csv file) that contains the data with at least 2 columns: an independent (grouping) variable and a dependent variable. Specify the decimal symbol (e.g., your text file stores numbers either as 3.14 or 3,14)
...
require(WRS2)
yuentest<-yuenbt(alldata[[measurelabel]] ~ as.factor(alldata[[factorlabel]]), data=alldata,tr=0.2,nboot=599,side=T) #for details of this function, see Wilcox, 2012, p. 163).
...
The 20% trimmed means indicates a difference in `r ylabelstring` between the conditions (*M* = `r round(yuentest$diff, digits = 2)`, `r 100*ConfInt`% symmetric CI [`r round(yuentest$conf.int[1], digits = 2)`;`r round(yuentest$conf.int[2], digits = 2)`]) as analyzed using the Yuen-Welch test for 20% trimmed means, *t* = `r round(yuentest$test, digits = 2)`, *p* `r ifelse(yuentest$p.value>=0.001," = ", " < ")` `r ifelse(yuentest$p.value>=0.001,formatC(round(yuentest$p.value, digits = 3)), "0.001")`, Robust *d~t~* = `r ifelse(InAHurry!="YES",round(d_robust, digits = 2),"NOT CALCULATED")`,  `r 100*ConfInt`% CI = [`r ifelse(InAHurry!="YES",round(d_robust_ci_l, digits = 2), "NOT CALCULATED")`;`r ifelse(InAHurry!="YES",round(d_robust_ci_u, digits = 2), "NOT CALCULATED")`]). The observed data is `r surprising2` under the assumption that the null-hypothesis is true. This can be considered a `r effectsize2` effect.

###*Figure 1*. Means and `r 100*ConfInt`% CI, and violin plot
...
###*Figure 2*. Bar chart displaying means, individual datapoints, and `r 100*ConfInt`% CI
...
###*Figure 3*. Bar chart displaying `r 100*ConfInt`% CI
